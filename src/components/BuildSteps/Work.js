...
const Work = () => {
    ...
    const handleChange = (e, id) => {
        ...
        const updatedWorkList = workList.map((work) => {
            if (work.id === id) {
                const updatedWork = { ...work, id: uuidv4(), [name]: value };

                // Check if the "End Date" is greater than the "Start Date"
                if (name === "endDate" && new Date(updatedWork.startDate) >= new Date(updatedWork.endDate)) {
        ...
        setWorkList(updatedWorkList.filter((work) => work)); // Remove items with validation errors
    }
